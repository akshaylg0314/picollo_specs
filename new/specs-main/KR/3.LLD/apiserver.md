...
1. REST API를 제공하여 Piccolo Cloud와 통신하거나 직접(direct) artifact를 수신한다.
2. 수신된 YAML 형식의 artifact를 파싱하고 유효성을 검증한다.
3. 공통 모듈에 정의된 scenario, package, model 등의 구조로 변환한다.
4. 공통 ETCD 라이브러리를 사용하여 변환된 artifact를 저장하고 실시간 감시(watch) 기능을 제공한다.
5. gRPC를 통해 FilterGateway에 새로운 artifact 정보를 전달한다.
6. 실시간 알림 시스템으로 artifact의 등록, 변경, 삭제를 구독자에게 통지한다.
7. 보안 컨텍스트와 권한 검증을 통해 안전한 API 액세스를 보장한다.
8. API 호출 로깅 및 모니터링을 통해 시스템 사용 현황을 추적한다.
9. 대규모 artifact 처리를 위한 비동기 처리 메커니즘을 제공한다.

### Main Dataflow

1. REST API를 통해 외부 시스템(Piccolo Cloud 또는 직접 요청)으로부터 다양한 HTTP 메소드로 artifact를 수신한다.
2. 수신된 artifact의 유효성을 검증하고 허용된 보안 컨텍스트인지 확인한다.
3. YAML 형식의 artifact를 종류별로 파싱하여 공통 데이터 모델 구조체로 변환한다.
4. 공통 ETCD 라이브러리를 사용하여 변환된 artifact를 저장하고 변경 사항을 감시한다.
5. 새로운 scenario가 등록되면 gRPC를 통해 FilterGateway에 전달한다.
6. 저장된 artifact 정보를 구독하는 시스템에 실시간 알림을 전송한다.
7. 새로운 또는 갱신된 artifact 정보를 NodeAgent에 전달하여 필요한 노드 작업을 수행하게 한다.
8. API 호출 및 처리 결과를 로깅하고 모니터링 시스템에 통계 정보를 전달한다.
9. 대규모 artifact 처리 시 비동기 큐를 활용하여 시스템 안정성을 보장한다.
...
- **main.rs** - API Server 초기화 및 실행
- **manager.rs** - 각 모듈 간의 데이터 흐름 제어 및 작업 조정
- **artifact/mod.rs** - Artifact 관리 및 조정
- **artifact/data.rs** - 공통 ETCD 라이브러리를 사용하여 파싱된 결과를 저장하거나 불러오는 함수 구현
- **artifact/parser.rs** - YAML 형식 문자열을 구조체로 파싱하고 변환
- **artifact/validation.rs** - Artifact 유효성 검증 및 검사
- **artifact/watcher.rs** - ETCD를 통한 Artifact 변경 감시 및 알림
- **grpc/mod.rs** - gRPC 통신 관리
- **grpc/sender/mod.rs** - gRPC 메시지 송신 관리
- **grpc/sender/nodeagent.rs** - NodeAgent와의 gRPC 통신
- **grpc/sender/filtergateway.rs** - FilterGateway와의 gRPC 통신
- **grpc/sender/notification.rs** - 알림 서비스를 위한 gRPC 통신
- **metrics/mod.rs** - 메트릭 관리 및 조정
- **metrics/collector.rs** - API 호출 및 성능 메트릭 수집
- **route/mod.rs** - REST API 라우팅 관리
- **route/api.rs** - REST API 핸들러 함수
- **route/auth.rs** - API 인증 및 권한 부여
- **route/middleware.rs** - API 미들웨어(로깅, 인증, 메트릭 등)
- **security/mod.rs** - 보안 관리
- **security/context.rs** - 보안 컨텍스트 및 세션 관리
- **security/permissions.rs** - API 권한 및 접근 제어
- **storage/mod.rs** - 스토리지 관리
- **storage/etcd_client.rs** - 공통 ETCD 라이브러리를 사용한 데이터 저장 및 검색
